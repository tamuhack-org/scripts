...
print("-- TAMUhack Team Identifier Script (Software/Hardware Edition) --")
...
# Load devpost tracks
print("Parsing devpost tracks...")
tracks = set()
software_tracks = set()
hardware_tracks = set()
with open(f"{SOFTWARE_TRACKS_FILENAME}") as in_file:
    for line in in_file.readlines():
        track_name = line.strip()
        software_tracks.add(track_name)
        tracks.add(track_name)

with open(f"{HARDWARE_TRACKS_FILENAME}") as in_file:
    for line in in_file.readlines():
        track_name = line.strip()
...
with open(DEVPOST_PROJECTS_CSV_FILENAME, encoding="utf-8") as file:
    with open(DEVPOST_PROJECTS_CSV_FILENAME, encoding="utf-8") as file2:
        ...
        for row in reader:
            ...
            project_tracks = row["Opt-In Prizes"].strip().split(", ")
            row["__is_software_project"] = False
            row["__is_hardware_project"] = False
            ...
            for track in project_tracks:
                if track in software_tracks:
                    ...
                    row["__is_software_project"] = True
                if track in hardware_tracks:
                    is_hardware_project = True
                    row["__is_hardware_project"] = True

            submission = {
                "title": row["Project Title"],
                "url": row["Submission Url"],
                "categories": row["Opt-In Prizes"].split(", "),
                "is_software_project": is_software_project,
                "is_hardware_project": is_hardware_project,
                "members": [
                    {
...
for submission in submissions:
    for member in submission["members"]:
        ...
        # Report if members are not in the right ware
        if member_application is not None:
            if submission["is_software_project"] and member_application["wares"] != "SW":
                member["wares"] = "Wrong ware: " + member_application["ware"]
                members_wrong_ware.append(member)
            if submission["is_hardware_project"] and member_application["wares"] != "HW":
                member["wares"] = "Wrong ware: " + member_application["ware"]
                members_wrong_ware.append(member)
...
for member in members_not_found:
    print(f"{member['firstName']} {member['lastName']} - {member['email']} - {member['wares']}")

print("\n\n\nThe following team members never checked in at the event!")
for member in members_not_checked_in:
    print(f"{member['firstName']} {member['lastName']} - {member['email']} - {member['wares']}")

print("\n\n\nThe following team members were in the wrong ware!")
for member in members_wrong_ware:
    print(f"{member['firstName']} {member['lastName']} - {member['email']} - {member['wares']}")
